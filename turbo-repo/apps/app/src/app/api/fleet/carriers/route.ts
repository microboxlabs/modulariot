...
export async function GET(request: Request) {
  ...
  try {
    const carriers = await client.fleet.listCarriers({ page, size });
    return NextResponse.json(carriers);
  } catch (error) {
    const status = error instanceof MiotResourceApiError ? error.status : 500;
    logger.error({ err: error }, "Failed to fetch carriers");
    return NextResponse.json({ error: "Failed to fetch carriers" }, { status });
  }
}
