...
| **miot-modulith** | Quarkus backend modulith (fleet, driver, tracking, gateway) | 0.4.0 |
...
```
      ...
      ▼              ▼                ▼               ▼              ▼
┌───────────┐ ┌───────────┐  ┌──────────────┐ ┌───────────┐ ┌────────────┐
│ miot-app  │ │ miot-docs │  │miot-modulith │ │miot-web-  │ │miot-gateway│
│ (Next.js) │ │ (Nextra)  │  │  (Quarkus)   │ │   site    │ │ (Quarkus)  │
│ Port:3000 │ │ Port:3000 │  │  Port:8180   │ │ Port:3000 │ │ Port:8180  │
└───────────┘ └───────────┘  └──────────────┘ └───────────┘ └────────────┘
                               fleet+driver     (Next.js)    gateway only
                               +tracking                     (same image)
```

The modulith and gateway deployments use the same `miot-modulith` chart and `miot-srv` image with different component flags.
