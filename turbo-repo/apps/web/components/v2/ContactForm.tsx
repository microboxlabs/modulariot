...
const COPY: Record<
  Lang,
  {
    ...
    sending: string;
    ctxTitle: string;
    ctx: { t: string; d: string; href: string }[];
    note: string;
  }
> = {
  es: {
    ...
    subtitle:
      "Cuéntanos qué necesitas y te respondemos en menos de 24 horas. Elige el tipo de conversación para que lleguemos preparados.",
    tabs: {
      demo: "Agendar demo",
      cotizar: "Cotizar / propuesta",
    ...
    required: "Completa nombre y email para continuar.",
    sending: "Abriendo tu correo…",
    ctxTitle: "Lo que verás con ModularIoT",
    ctx: [
      {
        t: "Síntomas en la Torre de Control",
        d: "36 reglas de detección sobre una operación real.",
        href: "/torre",
      },
      {
        t: "Notificaciones en tu canal",
        d: "La alerta en correo, WhatsApp, Teams, Webex o SMS.",
        href: "/canales",
      },
      {
        t: "SuperProfile",
        d: "La identidad operacional viva de cada actor.",
        href: "/superprofile",
      },
      {
        t: "Precisión de señal GPS",
        d: "12/20 pulsos por minuto contra el estándar.",
  ...
  en: {
    ...
    subtitle:
      "Tell us what you need and we'll reply within 24 hours. Pick the type of conversation so we come prepared.",
    tabs: {
      demo: "Book a demo",
      cotizar: "Get a quote",
    ...
    submit: "Send message",
    required: "Enter name and email to continue.",
    sending: "Opening your email…",
    ctxTitle: "What you'll see with ModularIoT",
    ctx: [
      {
        t: "Symptoms in the Control Tower",
        d: "35 detection rules on a real operation.",
        href: "/torre",
      },
      {
        t: "Notifications in your channel",
        d: "The alert in email, WhatsApp, Teams, Webex or SMS.",
        href: "/canales",
      },
      {
        t: "SuperProfile",
        d: "The living operational identity of each actor.",
        href: "/superprofile",
      },
      {
        t: "GPS signal precision",
        d: "12/20 pulses per minute against the standard.",
        href: "/proveedores-gps",
      },
    ],
  ...
  pt: {
    ...
    subtitle:
      "Conte o que precisa e respondemos em menos de 24 horas. Escolha o tipo de conversa para chegarmos preparados.",
    tabs: {
      demo: "Agendar demo",
      cotizar: "Cotar / proposta",
    ...
    f: {
      ...
      empresa: "Empresa",
      cargo: "Cargo (opcional)",
      flota: "Tamanho da frota",
      interes: "O que te interessa?",
      mensaje: "Mensagem",
      motivo: "Motivo",
    ...
    required: "Preencha nome e e-mail para continuar.",
    sending: "Abrindo seu e-mail…",
    ctxTitle: "O que você verá com o ModularIoT",
    ctx: [
      {
        t: "Sintomas na Torre de Controle",
        d: "35 regras de detecção sobre uma operação real.",
        href: "/torre",
      },
      {
        t: "Notificações no seu canal",
        d: "O alerta em e-mail, WhatsApp, Teams, Webex ou SMS.",
        href: "/canales",
      },
      {
        t: "SuperProfile",
        d: "A identidade operacional viva de cada ator.",
        href: "/superprofile",
      },
      {
        t: "Precisão de sinal GPS",
        d: "12/20 pulsos por minuto contra o padrão.",
        href: "/proveedores-gps",
      },
    ],
...
export default function ContactForm({
  ...
  const mailto = useMemo(() => {
    ...
    if (intent === "cotizar" && interes.length)
      lines.push(`Interés: ${interes.join(", ")}`);
  ...
  return (
    <Section tone={tone} contentClassName="py-12 px-4 sm:px-6">
      <div className="max-w-2xl">
        ...
        <p className="text-ink-2 mt-5 text-lg leading-relaxed">{t.subtitle}</p>
      </div>
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="border-hairline bg-surface inline-flex flex-wrap gap-1 rounded-lg border p-1">
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <button
                type="submit"
                className="border-ink-1 bg-ink-1 text-page hover:bg-ink-2 hover:border-ink-2 rounded-lg border px-6 py-3 text-sm font-medium transition-colors"
              >
                {t.submit}
              </button>
              <p className="text-ink-3 text-sm">{t.note}</p>
            </div>
          </form>
        </div>
        ...
        <aside className="lg:col-span-1">
          <div className="border-hairline bg-surface-2/60 rounded-xl border p-6">
            <p className="text-ink-1 text-sm font-semibold">{t.ctxTitle}</p>
            <ul className="mt-4 space-y-4">
              {t.ctx.map((c) => (
                <li key={c.t}>
                  <a href={`${base}${c.href}`} className="group block">
                    <span className="text-ink-1 group-hover:text-accent flex items-center gap-1.5 text-sm font-semibold">
                      {c.t}
                      <svg
                        className="text-accent h-3.5 w-3.5 opacity-0 transition-opacity group-hover:opacity-100"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth={2}
                    ...
                    <span className="text-ink-3 mt-0.5 block text-sm leading-relaxed">
                      {c.d}
                    </span>
                  </a>
                </li>
