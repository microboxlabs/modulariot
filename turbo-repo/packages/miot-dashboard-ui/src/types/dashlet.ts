 ...
 *
 * Moved from apps/app `features/dashboard/dashlets/types.ts` in P1, with one
 * addition: the optional `configSchema` slot (zod) on `DashletDefinition` —
 * the machine-readable config contract that dashlet authors fill in from P5
 * onward and that the AI dashboard-generation skill validates against.
 */

import type { ComponentType, ReactNode } from "react";
import type { ZodTypeAny } from "zod";
import type { Widget, DashletCategory } from "./dashboard";
...
export interface DashletDefinition {
  ...
  /**
   * Optional zod schema for this dashlet's `config` object. When present it
   * is the machine-readable contract used to validate generated/imported
   * configs (AI skill, batch import) and to emit JSON Schema artifacts.
   */
