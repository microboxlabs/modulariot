...
export function ConceptGraphic({ id }: { id: string }) {
  ...
  if (id === "ingesta") {
    return (
      <svg viewBox="0 0 400 176" className={banner} fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ingesta de señales GPS hacia tu base de datos">
        <Defs uid={uid} />
        <rect width="400" height="176" rx="16" fill={`url(#${uid}-blueSoft)`} />
        {/* nodo de señal */}
        <g filter={`url(#${uid}-shadow)`}>
          <circle cx="66" cy="66" r="20" fill={`url(#${uid}-blue)`} />
        </g>
        <path d="M66 52a14 14 0 010 28M66 58a8 8 0 010 16" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" fill="none" opacity="0.9" />
        <circle cx="66" cy="66" r="3.5" fill="#fff" />
        <circle cx="66" cy="66" r="34" stroke="#3f83f8" strokeWidth="2" opacity="0.18">
        ...
        {/* flujo de datos */}
        <line x1="98" y1="92" x2="300" y2="92" stroke="#3f83f8" strokeWidth="2.5" strokeDasharray="1 8" strokeLinecap="round" opacity="0.35" />
        {[120, 165, 210, 255].map((x, i) => (
        ...
        {/* base de datos */}
        <g filter={`url(#${uid}-shadow)`} transform="translate(300 52)">
  ...
  if (id === "sintomas") {
    return (
      <svg viewBox="0 0 400 176" className={banner} fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Motor de reglas que detecta síntomas por severidad">
        <Defs uid={uid} />
        <rect width="400" height="176" rx="16" fill="#f9fafb" />
        {/* radar */}
        ...
        <g transform="translate(214 46)" filter={`url(#${uid}-shadow)`}>
          <rect x="-8" y="-10" width="180" height="104" rx="12" fill="#fff" />
        </g>
  ...
  return (
    <svg viewBox="0 0 400 176" className={banner} fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Streaming de video en vivo desde cámaras">
      ...
      {/* pantalla */}
      <g filter={`url(#${uid}-shadow)`}>
        <rect x="40" y="36" width="188" height="112" rx="14" fill={`url(#${uid}-dark)`} />
      </g>
      <rect x="52" y="48" width="164" height="72" rx="8" fill="#0e1013" />
      {/* botón play */}
      <g filter={`url(#${uid}-shadow)`}>
      ...
      {/* etiqueta LIVE */}
      <rect x="60" y="128" width="42" height="12" rx="6" fill="#e11d48" opacity="0.9" />
      <circle cx="70" cy="134" r="3" fill="#fff">
        <animate attributeName="opacity" values="1;0.2;1" dur="1.4s" repeatCount="indefinite" />
      </circle>
      <rect x="150" y="129" width="60" height="8" rx="4" fill="#4b5563" />
      {/* tira de frames */}
      {[0, 1, 2, 3].map((i) => (
        <rect key={i} x={248} y={46 + i * 27} width="116" height="19" rx="6" fill="#3f83f8">
