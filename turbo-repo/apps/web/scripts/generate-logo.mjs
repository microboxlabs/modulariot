 ...
 *   app/icon.svg               favicon mark, adaptive, no background tile
 *   app/favicon.ico            static light-surface fallback, transparent bg
 *   packages/ui/src/brand/lynx-geometry.ts   path data for the React components
 *
...
// Favicon mark — just the Lynx, transparent background, no tile. Cropped tighter
// than MARK_VB (which carries the lockup's wider margins) and recentered on the
// mark's own bounding box — the tufts rise higher than the body sits low, so the
// geometric origin isn't the visual center. SVG favicons are re-rasterized by the
// browser so they can stay adaptive (prefers-color-scheme); favicon.ico is a
// static raster fallback and uses the light-surface (ink) colors.
const ICON_VB = "-120 -150 240 240";

function iconMarkSvg({ adaptive, ink, amber }) {
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${ICON_VB}" role="img" aria-label="ModularIoT">\n` +
    geomComment +
    (adaptive ? adaptiveStyle : "") +
    markBody(adaptive, ink, amber) +
    `</svg>\n`
...
const iconSvg = iconMarkSvg({ adaptive: true });
...
// Packs a single PNG image into a minimal .ico container (ICONDIR +
// one ICONDIRENTRY + the raw PNG bytes — the format modern browsers expect).
function pngToIco(png, size) {
  ...
  entry.writeUInt32LE(png.length, 8); // resource size
  entry.writeUInt32LE(header.length + entry.length, 12); // resource offset
...
out("../../packages/ui/src/brand/lynx-geometry.ts", geometryTs);

const FAVICON_SIZE = 64;
const faviconPng = await sharp(Buffer.from(iconMarkSvg({ adaptive: false, ink: INK, amber: AMBER })))
  .resize(FAVICON_SIZE, FAVICON_SIZE)
  .png()
  .toBuffer();
out("app/favicon.ico", pngToIco(faviconPng, FAVICON_SIZE));
