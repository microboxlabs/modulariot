...
export function ReviewSettingsDrawer({
  ...
  function patchChannel(key: string, patch: Partial<ChannelDraft>) {
    setChannels((prev) =>
      prev.map((c) =>
        channelKey(c.connectionId, c.operationId) === key
          ? { ...c, ...patch }
          : c
      )
    );
  ...
  return (
    <SettingsDrawerShell
      show={show}
      onClose={onClose}
      title={tr("drawer.title", dict)}
      subtitle={tr("drawer.subtitle", dict, { lane: laneTitle })}
      closeLabel={tr("drawer.close", dict)}
