...
/**
 * The org's bindings for one event type — the generic /bindings feed this
 * module fronts for every calendar-administered binding (enrichment here, the
 * assignment dispatch in ../dispatch). `T` narrows the row shape to whichever
 * optional columns that event uses.
 */
export async function fetchBindingsByEvent<T extends EnrichmentBinding>(
  orgSlug: string,
  eventType: string
): Promise<T[]> {
  ...
  const all = await unwrap<T[]>(response);
  return all.filter((binding) => binding.eventType === eventType);
}

/** Create-or-replace one binding row (any event — the body names it). */
export async function upsertBinding<T>(
  orgSlug: string,
  request: T
...
/** Delete one binding row by id (any event). */
...
export async function fetchEnrichmentBindings(
  ...
  return fetchBindingsByEvent(orgSlug, ENRICHMENT_EVENT_TYPE);
}
...
export async function upsertEnrichmentBinding(
  ...
  return upsertBinding(orgSlug, request);
}
...
export async function deleteEnrichmentBinding(
  ...
  return deleteBinding(orgSlug, bindingId);
}
