...
### 2.x. Fleet Visualization (UI package, Deck.GL + Mapbox)

- Objective: Provide a base Fleet Map in `packages/ui/src/fleets` with test pins, clickable tooltips and filtering, reusing the legacy project’s map behaviors at a minimal viable level.

- Scope:
  1) Testing data: Provide `data/test-positions.ts` array to seed the map.
  2) Map + DeckGL: `map/Map.tsx` uses `react-map-gl` + `deck.gl` to render the base map (Mapbox).
  3) Layers: `map/layers/pins-layer.ts` renders pins with status-based colors.
  4) Tooltips: `map/tooltip.tsx` opens on pin click with asset metadata.
  5) Filtering: `filters/filter-bar.tsx` filters by asset and status.

- Folder structure (UI):
  packages/ui/src/fleets
  ├─ fleets.tsx                    # container: loads test data, filters, tooltip state
  ├─ data/test-positions.ts       # testing positions
  ├─ filters/filter-bar.tsx       # search + status filter
  └─ map/
     ├─ Map.tsx                   # react-map-gl + DeckGL orchestration
     ├─ tooltip.tsx               # base tooltip
     └─ layers/
        └─ pins-layer.ts          # deck.gl scatterplot for pins

- Integration:
  - `packages/ui/src/fleets/map/fleet-view.tsx` renders `<Fleets />`.
  - Mapbox key via `NEXT_PUBLIC_MAPBOX_API_KEY`.
  - Initial centroid computed from test data, easing with FlyToInterpolator.

- Acceptance:
  - Pins visible, clickable tooltips, working search and status filters.
  - No TypeScript errors; `npm run build` succeeds in UI package.
