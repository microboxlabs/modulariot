...
export type { StatItem } from "./StatsGrid.types";
...
export function StatsGrid({
  ...
  const valueClass =
    tone === "dark" ? "text-page dark:text-ink-1" : "text-ink-1";
  const labelClass =
    tone === "dark" ? "text-page/60 dark:text-ink-3" : "text-ink-3";
  ...
  return (
    <div
      className={`grid gap-x-8 gap-y-10 ${colsClass} ${align === "center" ? "text-center" : ""}`}
    >
      {items.map((s, i) => (
        <Reveal key={s.label} delay={i * 0.06}>
          {s.prefix && <p className="text-ink-3 text-sm">{s.prefix}</p>}
          <p
            className={`display tabular-nums ${s.prefix ? "mt-1" : ""} ${valueSize} ${valueClass}`}
          >
            <Counter value={s.value} />
          </p>
          <p
            className={`mt-2 text-xs font-medium tracking-[0.08em] uppercase ${labelClass}`}
          >
            {s.label}
          </p>
        </Reveal>
