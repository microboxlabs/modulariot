...
export function Dashlet({ widget }: Readonly<DashletComponentProps>) {
  ...
  const resolvedRepresentations = useMemo(
    () =>
      (config.representations ?? []).map((r) => ({
        ...r,
        label: resolveHandlebarsField(r.label, templateContext),
      })),
  ...
  const resolvedConfig = useMemo(
    () => ({
      ...
      representations: resolvedRepresentations,
      xAxisDateFormat: effectiveDateFormat,
    }),
    [config, resolvedXAxisLabel, resolvedYAxisLabel, resolvedRepresentations, effectiveDateFormat]
  );
