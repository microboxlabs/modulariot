...
function StepperMarker({ id, selected, text, icon, current_step }: { id: number, selected: boolean, text: string, icon: React.ReactNode, current_step: number }) {
  return (
    <div className={`flex items-center justify-center flex-col gap-2 ${current_step > id ? "text-gray-300" : selected ? "text-blue-500" : "text-gray-500"}`}>
      <div className={`relative w-[10vh] portrait:w-[10vw] h-[10vh] portrait:h-[10vw] rounded-full border-4 flex items-center justify-center ${current_step > id ? "border-gray-300" : selected ? "border-blue-500" : "border-gray-500"}`}>
        {icon}
        <div className={`absolute top-[-1.5vh] portrait:top-[-1.5vw] right-[-1.5vh] portrait:right-[-1.5vw] w-[5vh] portrait:w-[5vw] h-[5vh] portrait:h-[5vw] bg-blue-300 rounded-full transition-opacity duration-300 ${current_step > id ? "opacity-100" : "opacity-0"}`}>
          <RxCheck className="w-full h-full text-blue-900" />
        </div>
      </div>
      <p className={"text-[2vh] portrait:text-[2vw] font-light whitespace-nowrap"}>{text}</p>
    </div>
  );
}