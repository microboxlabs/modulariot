...
const logoImgCls =
  "h-9 w-auto shrink-0 opacity-60 grayscale transition-opacity hover:opacity-100 hover:grayscale-0";
...
export async function TrustedBy({ lang, tone }: { lang: string; tone: Tone }) {
  ...
  const strip = useMarquee
    ? Array.from({ length: REPEAT }, () => CLIENTS).flat()
    : CLIENTS;
