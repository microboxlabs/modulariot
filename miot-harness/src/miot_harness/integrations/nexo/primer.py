"""Domain primer concatenated to every coordinador_analyst system prompt.

Kept as a module-level string so Anthropic prompt caching can mark this
block as `cache_control: ephemeral` and re-use it across runs (review
item S8). Updates here are observable in the next run; do not derive
