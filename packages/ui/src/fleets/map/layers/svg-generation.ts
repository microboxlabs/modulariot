...
function get_face(state: string, lost_signal: boolean = false) {
  return lost_signal ? disconnected_pin.face : PinState[state as keyof typeof PinState]?.face;
}
...
export function createSVGIcon(
  ...
  const color = PinState[state as keyof typeof PinState]?.main_color || "#000000";
