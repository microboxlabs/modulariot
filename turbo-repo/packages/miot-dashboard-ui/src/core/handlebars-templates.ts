...
import {
  ...
  type DashboardHandlebars,
} from "./handlebars-format-helpers";
import type { DataProviderEntry } from "../types/dashlet";
...
export function compileTemplates(
  ...
  hb: DashboardHandlebars = getDashboardHandlebars()
): Map<string, TemplateDelegate> {
  const map = new Map<string, TemplateDelegate>();
  for (const { id, template } of fields) {
    if (template.includes("{{")) {
      try {
        map.set(id, hb.compile(template));
      } catch {
...
export function resolveTemplate(
  compiled: Map<string, TemplateDelegate>,
  id: string,
