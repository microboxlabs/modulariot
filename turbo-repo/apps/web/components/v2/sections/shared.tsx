...
export function Section({
  ...
  return (
    <section
      id={id}
      className={`relative scroll-mt-16 overflow-hidden ${toneClasses[tone]} ${className}`}
    >
      ...
      <div className={`relative mx-auto max-w-7xl px-6 ${contentClassName}`}>
        {children}
      </div>
    </section>
  );
...
export function Eyebrow({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-accent inline-flex items-center gap-2 text-xs font-semibold tracking-[0.1em] uppercase">
...
export function SectionHeader({
  ...
  return (
    <Reveal className="max-w-[800px]">
      ...
      <h2 className="display mt-4 text-[clamp(30px,3.8vw,46px)] leading-[1.1] whitespace-pre-line">
        {title}
      </h2>
      {subtitle && (
        <p className="text-ink-2 mt-4 max-w-[56ch] text-[17px] leading-relaxed whitespace-pre-line">
          {subtitle}
        </p>
      )}
    </Reveal>
